...
BrdUwrapper <- source("BrdUwrapper.R")
                    ...
                    sidebarPanel(style = "max-height:10px; left: 0px; right: 0px; margin-bottom: 0px",
                                 ...
                                 fluidRow(
                                   ...
                                   column(4, align="center", offset = 0,
                                          useShinyjs(),
                                          extendShinyjs(text = "shinyjs.refrescar = function() { {history.go(0); }", functions = c("refrescar")),
                                          actionButton("cRefrescar", "Refrescar App", icon=icon("refrescar"), width = 250, 
      ...
      fluidRow(
        ...
        column(4, offset = -1,
               ...
               sidebarPanel(style = "max-height: calc(100vh - 18rem); overflow-y: auto;",
                            ...
                            textInput("BrdUbin", "Bin (default recommended)", placeholder = "defaults to MFS"),
                            
                            textInput("BrdUsteps", "Slider steps (sliding windows)", placeholder = "10"),
                            
                            ##
                            
                            radioButtons("BrdUprofile", "Plot IP Profile",
                                         choiceNames = list(
                                           "Genomewide", "Fixed-coordinates", "Both"
                                         ),
                                         choiceValues = list(
                            ...
                            textInput("BrdUaveWin", "Range (upstream:downstream)", placeholder = "e.g. 3000:3000"),
        ...
        column(8, offset = -1, style='padding-left:0px; padding-top:0px;',
               ...
               sidebarPanel(style="max-height: 113px;",
                            ...
                            tabsetPanel(
                              ...
                              tabPanel("SUP_R1", textInput("text11", h1(),  value = NULL, placeholder = "/Enter/full/path/to/fastq.gz ...")),
                              
                              tabPanel("SUP_R2", textInput("text12", h1(),  value = NULL, placeholder = "/Enter/full/path/to/fastq.gz ..."))
                    ...
                    sidebarPanel(style = "max-height:10px; margin-bottom: 0px;",
                                 ...
                                 fluidRow(
                                   ...
                                   column(4, align="center", offset = 0,
                                          useShinyjs(),
                                          extendShinyjs(text = "shinyjs.refrescar = function() { {history.go(0); }", functions = c("refrescar")),
                                          actionButton("eRefrescar", "Refrescar App", icon=icon("refrescar"), width = 250, 
                                                       style="color: #fff; background-color: #33cc33; border-color: #33cc33; font-weight: bold;")),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$go, {
      ...
      withProgress(message = 'Analysis in progress... (this will take time)',
                   detail = 'Go get some coffee...', value = 0, {
                     CBrdUwrapper(
                       ...
                       bin = input$BrdUbin,
                       stepSize = input$BrdUsteps, 
                       
                       PlotIPprofile = input$BrdUprofile,
                       ChromCoords = input$textCoords, 
                       
                       peakSet = input$peaklist,
                       AveragingPan = input$BrdUaveWin, 
                       Normalise = input$Norm
    ...
    message = function(m) {
      shinyjs::html(id = "BrdU", html = paste0(m$message,  sep="<br/>"), add = T)
    })
  })
